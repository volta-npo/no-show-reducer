...
export function calculateSaasMetrics(config, domain, state) {
    ...
    if (!nextBestActions.length)
        nextBestActions.push('Package this workspace as a repeatable SaaS onboarding motion.');
...
export function validateSaasState(config, domain, state) {
    ...
    if (!Array.isArray(state.accounts) || state.accounts.length < 3)
        throw new Error('saas state needs at least three accounts');
