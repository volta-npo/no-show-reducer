...
export function calculateSaasMetrics(config, domain, state) {
  ...
  if (onboardingCompletion < 75) nextBestActions.push('Finish onboarding milestones and clear blocked launch gates.');
  if (integrationReadiness < 80) nextBestActions.push('Verify every planned integration before promoting the workspace.');
  if (governanceScore < 85) nextBestActions.push('Review audit-log evidence and rollback owners for governance readiness.');
  if (retentionScore < 80) nextBestActions.push('Prioritize customer success outreach for medium/high-risk accounts.');
  if (!nextBestActions.length) nextBestActions.push('Package this workspace as a repeatable SaaS onboarding and expansion motion.');
...
export function validateSaasState(config, domain, state) {
  ...
  if (!Array.isArray(state.accounts) || state.accounts.length < 4) throw new Error('saas state needs at least four accounts');
  ...
  if (!Array.isArray(state.onboarding) || state.onboarding.length < 5) throw new Error('saas onboarding must cover the customer launch path');
  if (!Array.isArray(state.roleMatrix) || state.roleMatrix.length < 4) throw new Error('saas role matrix must cover operating personas');
  if (!Array.isArray(state.integrationHealth) || state.integrationHealth.length < 4) throw new Error('saas integrations must be tracked');
  if (!Array.isArray(state.successPlans) || state.successPlans.length < 4) throw new Error('saas success plans must be tracked');
  if (!Array.isArray(state.auditLog) || state.auditLog.length < 3) throw new Error('saas audit log must track governance events');
